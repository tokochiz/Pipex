...
void execute_first_command(t_pipex pipex, char **argv, char *envp[])
{
	...
	dup2(pipex.tube[1], 1);
	// TODO:　パイプがわの読み取りがわを閉じる
	close(pipex.tube[0]);
	// TODO: 入力ファイルを標準入力にリダイレクトする このプロセスで読み取るデータはfile１から来ることになる
	dup2(pipex.infile, 0);
	// TODO: 第２の引き数（cmd１）を空白文字で分割、結果をlこまんどとして保持
	pipex.cmd_args = ft_strcmp(argv[2], ' ');
	// TODO: コマンドを探す　なかったら異常終了する
	pipex.cmd_fullpath = get_cmd_execution_path(pipex.cmd_paths_list, pipex.cmd_args);
...
// TODO: パイプの読み込みがわを標準入力にする　標準出力：１
	...
	pipex.cmd_args = ft_strcmp(argv[3], ' ');
	// TODO: コマンドを探す　なかったら異常終了する
	pipex.cmd_fullpath = get_cmd_execution_path(pipex.cmd_paths_list, pipex.cmd_args);
	// 指定されたコマンドが見つからない場合にエラーメッセーじを表示する
